...
-- Obtener una lista de todas las razas únicas
SELECT @cols = STUFF((SELECT DISTINCT ',' + QUOTENAME(race)
                      FROM #FinalPrincipal
                      FOR XML PATH(''), TYPE
...
WITH RankedKi AS (
    SELECT
        race,
        KiFinal,
        ROW_NUMBER() OVER (PARTITION BY race ORDER BY KiFinal) AS RowAsc,
        ROW_NUMBER() OVER (PARTITION BY race ORDER BY KiFinal DESC) AS RowDesc,
        COUNT(*) OVER (PARTITION BY race) AS TotalCount
    FROM
        #FinalPrincipal
),
MedianKi AS (
    SELECT
        race,
        AVG(CAST(KiFinal AS FLOAT)) AS Median_KiFinal
    FROM
        RankedKi
    WHERE
        RowAsc = RowDesc
        OR (TotalCount % 2 = 0 AND (RowAsc = RowDesc + 1 OR RowAsc = RowDesc))
    GROUP BY
        race
)
SELECT * FROM (
